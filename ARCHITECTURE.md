...
├── bot/
│   ├── handlers/
│   │   ├── teacher/        # уведомления, быстрые действия, кнопка открытия Mini App
│   │   ├── student/        # запись на курс, сдача работ, получение фидбека
│   │   └── common/         # /start, /help
│   ├── middlewares/        # AuthMiddleware, RateLimitMiddleware
│   ├── keyboards/          # фабрики inline/reply клавиатур + WebApp-кнопки
│   └── states/             # FSM-состояния (aiogram FSMContext)
...
| `Course` | id, creator_id, name, invite_code, created_at |
| `CourseRole` | user_id, course_id, role (owner\|student), joined_at |
| `Assignment` | id, course_id, title, description, criteria, deadline |
| `Submission` | id, assignment_id, student_id, content_text, file_ids[], status |
...
/profile → сменить отображаемое имя в любой момент

Главное меню (динамическое):
  ➕ Создать курс         (только если в whitelist)
  🎓 Открыть панель       (только если в whitelist) → Mini App
  🎓 Мои курсы (преп.)   (если есть owner-курсы)
    → Уведомления о новых работах
  📚 Мои курсы (студент) (если есть student-курсы)
  🔑 Вступить в курс по коду
```
...
/profile → сменить имя
  ...
  → [Вступить в курс] → ввести invite-код → CourseRole(student) создаётся
  📚 Мои курсы
    → Открыть курс
        → Список заданий (дедлайн, статус сдачи)
        → Открыть задание
            → Условие задания
            → [Сдать работу] → отправить текст / файл / код
            → Статус: ⏳ На проверке / ✅ Проверено
            → [Посмотреть фидбек] — когда преподаватель отправил
```
...
1. Скаффолдинг: структура, `requirements.txt`, Docker, `core/config.py`, `core/security.py`
2. DB: SQLAlchemy модели + Alembic init + первая миграция
3. Bot: `/start` → запрос имени → регистрация; `/profile` → смена имени; `/admin add_teacher` для whitelist
4. Bot: курсы (создание → CourseRole owner, вступление по коду → CourseRole student)
5. Bot: задания (создание, список)
6. Bot (student): сдача работы (текст / файл)
